...
if (args[0].macroPass === 'postActiveEffects') {
    if (workflow.isCritical) {
        game.critCheckWorkflow.rollForCriticalEvents(workflow, "isCritical"); //Critical Hit for item with attack roll and damage roll
    } else if (workflow.isFumble) {
        game.critCheckWorkflow.rollForCriticalEvents(workflow, "isFumble"); //Critical Fumble for item with attack roll and damage roll
    } else if (workflow.fumbleSaves.size > 0) {
        game.critCheckWorkflow.rollForCriticalEvents(workflow, "isFumbledSave"); //Critical Fumble for item without attackroll and with saving throw
    }
...
if (args[0].macroPass === 'postActiveEffects') {
    if (workflow.isCritical) {
        ...
        game.critCheckWorkflow.rollForCriticalEvents(workflow, "isFumbledSave"); // Critical Fumble for item without attack roll and with saving throw
        ...
        for (const token of workflow.damageList) {
            ...
            if (roll.result === "20") {
                game.critCheckWorkflow.rollForCriticalEvents(workflow, "isCritical");
            } else if (roll.result === "1") {
                game.critCheckWorkflow.rollForCriticalEvents(workflow, "isFumble");
            }
        }
    }
} */
