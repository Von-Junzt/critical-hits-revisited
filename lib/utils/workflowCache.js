...
export const workflowCache = {
    ...
    async getCachedDialogResult() {
        const workflowItem = await workflowCache.getWorkflowItem();
        mainScriptUtils.debug('Dialog result retrieved: ', workflowItem.flags.cachedDialogResult);
        return workflowItem.flags.cachedDialogResult;
    },
    async deleteCachedDialogResult() {
        const workflowItem = await workflowCache.getWorkflowItem();
        await workflowItem.update({'flags.cachedDialogResult': null});
        mainScriptUtils.debug('Dialog result deleted.');
    }
};
