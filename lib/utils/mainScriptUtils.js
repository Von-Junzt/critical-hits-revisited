...
export const mainScriptUtils = {
    applyEffects: async function (effects, targetActorUuid, tableName) {
        ...
        const attackerToken = attackerActor.token;
        ...
        mainScriptUtils.debug("Applying effects:", debugObject);
        ...
        await Promise.all(effects.map(async effect => {
            ...
            if (!animationPlayed) {
                if (animationData[normalizedEffect]) {
                    ...
                    mainScriptUtils.debug("Playing animation:", normalizedEffect);
                    animationPlayed = true;
                } else if (animationData[tableName.toLowerCase()]) {
                    mainScriptUtils.debug("Playing animation:", tableName.toLowerCase());
