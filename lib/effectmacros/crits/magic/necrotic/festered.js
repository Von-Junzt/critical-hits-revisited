...
export async function festered(actor, effect) {
  ...
  const targetWorkflow = await workflowCache.getWorkflowByTargetUUID(actor.uuid);
  const targetDamage = targetWorkflow.damageList.find(entry => entry.actorUuid === actor.uuid).healingAdjustedTotalDamage || 0;
  await effect.update({
    changes: [{
