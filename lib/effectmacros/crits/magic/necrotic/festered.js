...
export async function festered(target, mode, workflow) {
    ...
    if (mode === 'add') {
        if (festeredEffect) {
            ...
            mainScriptUtils.debug('Existing Festered effect reverted', { actor: target.name, restoredHP: existingReduction });
        }
        ...
        mainScriptUtils.debug('New Festered effect applied', { actor: target.name, hpReduction: newHPReduction });
    } else if (mode === 'remove') {
        ...
        mainScriptUtils.debug("Festered effect removed", { actor: target.name, restoredHP: hpReduction });
    }
}