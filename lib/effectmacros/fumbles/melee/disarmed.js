 ...
 * @param fumbleToken {token} The token object, passed by the effect macro.
 * @param fumbleActor {actor} The actor object, passed by the effect macro. This is the AOOs target.
...
export async function disarmed(effect, fumbleToken, fumbleActor) {
    ...
    const weapon = await fromUuid(workflow.itemUuid);
    const weaponName = weapon.name.toLowerCase();
    if(!weapon || nonWeaponNames.some(name => weaponName.includes(name)) || weapon?.system?.type?.value === 'natural') {
    ...
    if (!userDecision) {
        ...
        for(const target of targetList) {
            if (MidiQOL.hasUsedReaction(target.actor)) {
                ChatMessage.create({ content: `${fumbleToken.name} is picking up his weapon. ${target.name} could make an opportunity attack but has already used their reaction this turn!`});
            ...
            if(!weapon) {
                ChatMessage.create({ content: `${fumbleToken.name} is picking up his weapon. ${target.name} can't make an opportunity attack because they have no melee weapon equipped!`});
            ...
            let popUpDialog = await foundry.applications.api.DialogV2.wait({
                ...
                content: `<p>${target.name} can make an attack of opportunity on your token.</p>`,
                buttons: [{
                    action: "confirm",
